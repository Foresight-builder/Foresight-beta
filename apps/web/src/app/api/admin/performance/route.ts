...
export async function GET(req: NextRequest) {
  try {
    ...
    // 如果邮箱检查失败，尝试通过钱包地址检查（如果 metadata 中有）
    if (!isAdmin && session.user.user_metadata?.wallet_address) {
      const { data: profile } = await client
        .from("user_profiles")
        .select("is_admin")
        .eq("wallet_address", session.user.user_metadata.wallet_address)
        .maybeSingle();
    ...
    // 4. 获取慢查询列表
    const { data: slowApis } = await client.from("slow_apis").select("*").limit(20);
