...
export default function ProposalsPage() {
  ...
  const activeProposalsCount = proposals.filter((p: any) => {
    const votes = Number(p.upvotes || 0) + Number(p.downvotes || 0);
    return votes > 10;
  }).length;
                ...
                <div className="text-lg font-black text-gray-800">
                  {
                    proposals.filter((p: any) => {
                      ...
                      return (
                        me && String(p.user_id || "").toLowerCase() === String(me).toLowerCase()
                      );
                    }).length
                  }
                </div>
