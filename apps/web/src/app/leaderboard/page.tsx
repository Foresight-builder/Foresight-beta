...
type LeaderboardUser = (typeof leaderboardData)[number];
...
const TopThreeCard = ({ user }: { user: LeaderboardUser }) => {
...
const RankItem = ({ user, index }: { user: LeaderboardUser; index: number }) => {
  return (
    <motion.div
...
const LeaderboardPodium = ({ users }: { users: LeaderboardUser[] }) => {
...
const LeaderboardMainSections = ({ restRank }: { restRank: LeaderboardUser[] }) => {
  return (
...
type LeaderboardPageViewProps = {
  ...
  jsonLd: ReturnType<typeof buildLeaderboardJsonLd>;
};

function LeaderboardPageView({
  timeRange,
  category,
  onTimeRangeChange,
  onCategoryChange,
