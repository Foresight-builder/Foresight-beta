...
export function createMockNextRequest(options: {
  ...
  const { method = 'GET', url = 'http://localhost:3000', headers = {}, body, cookies = {} } = options;
...
export const assertions = {
  ...
  assertHasField(obj: any, field: string) {
    if (!(field in obj)) {
      throw new Error(`Expected object to have field "${field}", but it doesn't. Object: ${JSON.stringify(obj)}`);
    }
  },
