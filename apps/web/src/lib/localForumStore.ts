...
async function saveThreads(records: Thread[]): Promise<void> {
  ...
  await fs.promises.writeFile(THREAD_PATH, json, "utf-8");
}

async function saveComments(records: Comment[]): Promise<void> {
  ensureDir();
  const json = JSON.stringify(records, null, 2);
  await fs.promises.writeFile(COMMENT_PATH, json, "utf-8");
}
...
export async function getThreadWithComments(
  eventId: number
): Promise<Array<Thread & { comments: Comment[] }>> {
